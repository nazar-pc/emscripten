...
.. js:attribute:: Module.locateFile
	...
	If set, this method will be called when the runtime needs to load either a file generated by the file packager. This is a generalization of ``Module.filePackagePrefixURL``, ``Module.memoryInitializerPrefixURL``, ``Module.cdInitializerPrefixURL``, ``Module.filePackagePrefixURL`` and is used if any of those are not defined. The function receives the file name, and should return the actual URL. This lets you host file packages or the ``.mem`` file on a different location than the current directory (which is the default expectation), for example if you want to host them on a CDN.

.. js:attribute:: Module.scriptDirectory

	Default to current directory of ``.js`` file and is used by built-on implementation of ``Module.locateFile`` (so instead of specifying the function that appends the same prefix to everything you can just define this settings). Effectively defines with specified value all of the following at the same time: ``Module.filePackagePrefixURL``, ``Module.memoryInitializerPrefixURL``, ``Module.cdInitializerPrefixURL`` and ``Module.filePackagePrefixURL``.
