...
.. js:attribute:: Module.scriptDirectory
	...
	Defaults to current directory of ``.js`` file and is used for ``Module.*PrefixURL`` if any of those are not specified explicitly, allows customizing the URL where .wasm, .mem and some other files are located.
